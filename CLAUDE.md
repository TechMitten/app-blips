...
- The LLM base URL, API key, model, `max_tokens`, temperature, and reasoning effort are **not** user-configurable in the UI — they're read server-side from `APPBLIPS_LLM_BASE_URL` / `APPBLIPS_LLM_API_KEY` / `APPBLIPS_LLM_MODEL` / `APPBLIPS_LLM_MAX_TOKENS` / `APPBLIPS_LLM_TEMPERATURE` / `APPBLIPS_LLM_REASONING_EFFORT` env vars (see `.env.example`). `APPBLIPS_LLM_TEMPERATURE` defaults to `0.2` when unset. None are `VITE_`-prefixed, so Vite never inlines them into the client bundle.
