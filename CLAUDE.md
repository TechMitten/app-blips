...
**Hosted mode:** enabled previews receive a render-time bridge whose requests cross the token-authenticated, source-checked `postMessage` channel; the parent calls authenticated `/api/chat`. Enabled deployments receive a deploy-time bridge containing a freshly rotated opaque token and call same-origin `/ai/chat`. The token is public by design and only binds/rate-limits calls to a real deployment. `functions/_lib/aiRelay.js` forces server provider settings from the dedicated `APPBLIPS_APP_LLM_*` variables (separate from the builder's `APPBLIPS_LLM_*`), caps payload/output, rejects cross-origin requests, and uses the `APPBLIPS_AI_RATE_LIMIT_*` best-effort token bucket. Hosted customers never see provider keys.
