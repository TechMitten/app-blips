---
title: "Troubleshooting: Fixes for Common Problems"
...
<AccordionGroup>
  <Accordion title="I get an API error when building">
    ...
    **Check the following in your `.env` file:**
    ...
    - Exactly one provider key is filled in (for example `APPBLIPS_OPENAI_API_KEY`), or `APPBLIPS_LLM_PROVIDER` picks one if several are
    - The key is valid and active for that provider
    - `APPBLIPS_LLM_MODEL` is a model name your provider recognizes and your key has access to
    - For another OpenAI-compatible service, `APPBLIPS_LLM_BASE_URL` is the full base URL (for example `http://localhost:11434/v1`)

    When AppBlips starts, it prints which provider it found and which values are still missing. Check that output first.
  ...
  <Accordion title="Build hangs and never completes">
    ...
    **Try the following:**
    ...
    3. If **Reasoning: building** or **Reasoning: editing** is set to High in Settings, try switching it to Low — reasoning takes longer and is more likely to hit timeouts on slower providers.
  </Accordion>
  ...
  <Accordion title="Rate limit error">
    ...
    **To fix:**
    ...
    - If you are running a self-hosted copy and need a higher limit, increase `APPBLIPS_CHAT_RATE_LIMIT_MAX` or `APPBLIPS_CHAT_RATE_LIMIT_WINDOW_SECONDS` in your `.env` file and restart the server.
