...
Reasoning has two separate choices, so you can have the AI think harder when building a new app without slowing down small edits.
...
<ParamField body="Reasoning: editing" type="Low | High" default="Low">
  Lets the AI reason before changing an existing app. **Low** keeps small tweaks fast; use **High** for tricky changes.
</ParamField>

<Tip>
  Chat-only replies (clarifying questions and Ask-mode answers) always run with reasoning off, so they stay fast.
</Tip>
