...
Reasoning has two separate switches, so you can have the AI think harder when building a new app without slowing down small edits.

<ParamField body="Reasoning: building" type="Off | On" default="Off">
  Lets the AI reason before generating a new app from scratch. Can improve complex apps, but is slower and uses more tokens.
</ParamField>

<ParamField body="Reasoning: editing" type="Off | On" default="Off">
  Lets the AI reason before changing an existing app. **Off** makes small tweaks faster; turn it on for tricky changes.
</ParamField>
