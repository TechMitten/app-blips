---
title: "blip.ai API Reference: text(), configure()"
...
Sends a list of messages to the configured AI provider and returns the model's reply. In self-hosted BYOK mode, if no provider has been connected yet, this call automatically opens the **Connect your AI provider** dialog before proceeding.
...
On appblips.com and in self-hosted relay mode, this resolves immediately without showing any dialog, because the provider is configured on the server.
