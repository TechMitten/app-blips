...
<ParamField body="APPBLIPS_LLM_ASK_MAX_TOKENS" type="number">
  Maximum number of tokens the LLM may return in a single Ask-mode chat reply. Independent of `APPBLIPS_LLM_MAX_TOKENS`, which does not apply to Ask mode. Defaults to `8192`, which leaves headroom for reasoning tokens on thinking models.
</ParamField>
