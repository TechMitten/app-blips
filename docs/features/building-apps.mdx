...
AppBlips can let the AI think before it writes code. In **Settings → AI** there are two choices, each **Low** or **High**:

| Setting | What it affects |
|---------|---------------|
| **Reasoning: building** | Generating a new app from scratch |
| **Reasoning: editing** | Changing an existing app |

Both default to Low, which is fastest. Choose High for building apps with intricate logic or multi-step flows, and for editing when a change is tricky. Higher reasoning takes longer and uses more tokens.
