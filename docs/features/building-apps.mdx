...
AppBlips can let the AI think before it writes code. In **Settings → AI** there are two switches, each **Off** or **On**:
...
Both are off by default, which is fastest. Turn building on for apps with intricate logic or multi-step flows, and editing on when a change is tricky. Reasoning takes longer and uses more tokens.
