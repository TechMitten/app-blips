...
AppBlips runs LLM-generated code and can host it at public URLs, so these areas matter most:
...
In the default self-hosted mode, `/api/chat` performs no authentication, so anyone who can reach it can spend your configured LLM budget. That is by design for a single-user local setup. If you expose an installation beyond localhost, put your own access control in front of it. See the Security notes in the README.
